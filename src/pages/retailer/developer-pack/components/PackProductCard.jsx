      ...
      <CardContent className="p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2 bg-surface-muted p-2 rounded-md border border-border/50">
          <div className="flex flex-col">
            <span className="text-2xs text-text-muted flex items-center gap-1"><Package size={12}/> Qty</span>
            <span className="font-bold text-sm text-text-primary">{product.suggestedQuantity} &times; {product.variant || product.unit}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-2xs text-text-muted flex items-center gap-1"><Tag size={12}/> Est. Price</span>
            <span className="font-bold text-sm text-primary">
              ₹{Math.round(product.lineTotal ?? product.price).toLocaleString('en-IN')}
            </span>
            <span className="text-2xs text-text-muted">₹{product.price.toLocaleString('en-IN')} per pack</span>
          </div>
          <div className="col-span-2 pt-1 border-t border-border/50 mt-1 flex items-center gap-1 flex-wrap">
             ...
             <span className="text-2xs text-text-muted">MOQ: {product.minimumOrderQuantity} packs</span>
             {product.distributorName && (
               <span className="text-2xs text-text-muted">&middot; {product.distributorName}</span>
             )}
          </div>
        </div>
